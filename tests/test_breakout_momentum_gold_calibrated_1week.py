...
from nero_core.strategies.breakout_momentum import STRATEGY_ID, size_entry
...
from nero_core.strategies.mean_reversion_gold_calibrated_1week import WEEKLY_MAX_HOLDING_HOURS
...
class OneWeekRegressionTest(unittest.TestCase):
    """Same regression as MEAN_REVERSION's 1week fix: the old (24h) cap force-closes a
    trade via TIME on the very next 1week candle no matter what price did; the fixed cap
    does not."""
    ...
    def test_old_cap_forces_time_exit_on_the_very_next_weekly_candle(self) -> None:
        ...
        next_weekly_candle = make_candle(close_time=7 * 24 * 3600000, high=110.5, low=109.0, close=110.0)
...
class OneWeekRegistrationTest(unittest.TestCase):
    def test_register_default_variant_uses_correct_id_and_version(self) -> None:
        ...
        self.assertEqual(variant.version, "breakout-momentum-v1.2.0-gold-calibrated-1week")
    ...
    def test_registers_alongside_gold_calibrated_as_a_separate_version(self) -> None:
        ...
        self.assertIn("breakout-momentum-v1.1.0-gold-calibrated", versions)
        self.assertIn("breakout-momentum-v1.2.0-gold-calibrated-1week", versions)
