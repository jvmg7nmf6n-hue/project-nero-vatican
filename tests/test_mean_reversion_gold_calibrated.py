...
class GoldFeeScaleFactorTest(unittest.TestCase):
    def test_scale_factor_matches_measured_ratio_division(self) -> None:
        self.assertAlmostEqual(GOLD_FEE_SCALE_FACTOR, BTC_MEASURED_PRICE_ATR_RATIO / GOLD_MEASURED_PRICE_ATR_RATIO)

    def test_scale_factor_is_less_than_one(self) -> None:
        # GOLD's measured price/ATR is higher than BTC's (lower relative volatility),
        # so the fee scale-down factor must be < 1.
        self.assertLess(GOLD_FEE_SCALE_FACTOR, 1.0)
        self.assertGreater(GOLD_FEE_SCALE_FACTOR, 0.0)

    def test_fee_bps_and_slippage_bps_scaled_by_the_measured_factor(self) -> None:
        ...
        # sanity: both should be meaningfully lower than v1's crypto-calibrated defaults
...
class GoldCalibratedEntryExitLogicUnchangedTest(unittest.TestCase):
    """Proves the GOLD-calibrated variant reuses v1's entry/exit logic exactly — only
    fee_bps/slippage_bps differ; every other parameter and every entry decision must be
    identical to v1 for the same input."""
    ...
    def test_entry_evaluation_rejection_reasons_identical_for_a_failing_candle(self) -> None:
        candle = make_candle(rsi=80.0, ma200=150.0)
        state_v1 = MeanReversionState(equity=10000.0)
        state_gold = MeanReversionState(equity=10000.0)
    ...
    def test_stop_distance_identical_lower_fees_only(self) -> None:
        ...
        # entry price — and therefore the absolute stop/target price — shifts slightly.
        # What must stay IDENTICAL is the risk distance itself: atr_stop_multiple * ATR,
        # i.e. the actual stop-loss rule, untouched by the fee/slippage recalibration.
        candle = make_candle()
        state_v1 = MeanReversionState(equity=10000.0)
        state_gold = MeanReversionState(equity=10000.0)
        ...
        # the whole point of the recalibration: strictly lower fees for the same trade
...
class GoldCalibratedRegistrationTest(unittest.TestCase):
    def test_register_default_variant_uses_correct_id_and_version(self) -> None:
        ...
        self.assertEqual(variant.version, "mean-reversion-v1.1.0-gold-calibrated")
    ...
    def test_registers_alongside_v1_as_a_separate_version_of_the_same_strategy(self) -> None:
        ...
        self.assertEqual(versions, {"mean-reversion-v1.0.0", "mean-reversion-v1.1.0-gold-calibrated"})
