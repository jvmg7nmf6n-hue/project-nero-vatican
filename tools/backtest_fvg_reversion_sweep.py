"""CLI: backtest FVG_REVERSION (nero_core.strategies.fvg_reversion) across 7 crypto
assets on 2h/4h/12h timeframes, through the upgraded statistical harness (bootstrap 95%
CI + random-entry baseline), with a chronological 70/30 train/test split, classified
into SURVIVED / PROMISING-WATCHLIST / DIED (tools.backtest_statistics.classify_verdict).

RANDOM-BASELINE ELIGIBLE POOL NOTE: FVG_REVERSION's size_entry needs the specific
touched gap's own zone data (only non-NaN on an actual touch candle) — a broad
"regime holds" random-entry pool would try to size an entry at a candle with no real
zone data at all and produce NaN. The eligible pool used here is therefore narrowed to
"has an actual signal, on the side the trend filter allows" — a random RESAMPLE of the
real trigger events (at the strategy's own realized trade-count rate), not an
unconstrained "any candle in the regime" pool like BREAKOUT_MOMENTUM/TREND_PULLBACK
get. This is a necessary adaptation for any trigger-derived-stop strategy (see
nero_core.strategies.bos_continuation for the same adaptation), documented rather than
silently narrowed.
...
Usage:
    python tools/backtest_fvg_reversion_sweep.py
"""
...
TIMEFRAMES = ["2h", "4h", "12h"]


def _half_stats(half_candles, params: FvgReversionParameters) -> dict[str, object]:
