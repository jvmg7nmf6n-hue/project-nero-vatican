...
describe("StrategyCard backtest evaluation indicator", () => {
  ...
  it("2026-08-03 production incident regression: never crashes when backtest_evaluation is entirely absent from the entry", () => {
    // Real incident: the live site's currently-fetched strategies.json (last
    // regenerated by the cloud scheduler BEFORE this field existed at all)
    // had NO backtest_evaluation key on any of its 37 entries -- not just a
    // missing sub-field, the whole key was `undefined` -- crashing every page
    // that read entry.backtest_evaluation.X directly with a 500. Simulates
    // that exact shape (a real StrategyRosterEntry with the key stripped,
    // not just a partial override) rather than trusting the fixture default.
    const { backtest_evaluation: _omit, ...entryWithoutBacktestEvaluation } = makeEntry();
    expect(() =>
      render(<StrategyCard entry={entryWithoutBacktestEvaluation as StrategyRosterEntry} recentRows={[]} stats={[]} />)
    ).not.toThrow();
    expect(screen.queryByTestId("card-backtest-verdict")).not.toBeInTheDocument();
    expect(screen.queryByTestId("card-backtest-untestable")).not.toBeInTheDocument();
    expect(screen.getByTestId("provenance-line")).toBeInTheDocument();
  });
});
