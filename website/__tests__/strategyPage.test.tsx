...
describe("StrategyDetailPage", () => {
  ...
  it("2026-08-03 production incident regression: never crashes when backtest_evaluation is entirely absent from the roster entry", async () => {
    ...
    // had NO backtest_evaluation key on any of its 37 entries -- crashed
    // every page with a 500. Strips the key entirely (not a partial
    // override) to reproduce the exact real shape.
    const { backtest_evaluation: _omit, ...entryWithoutBacktestEvaluation } = ROSTER_ENTRY;
    setupMocks({
      strategies: { schema_version: 1, last_updated: "x", strategies: [entryWithoutBacktestEvaluation] },
    });
